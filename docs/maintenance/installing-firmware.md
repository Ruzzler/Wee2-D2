...
1. **Connect**: Use a high-quality USB-C cable to connect your ESP32-S3 node to your computer.
...
4. **Monitor**: Watch the initialization logs to ensure the node successfully connects to your Wi-Fi network.
...
- **Verification**: The node will check the `ota_password` before initiating the write cycle.
...
| :--- | :--- | :--- |
| **Node 1** | `wee2d2-dome-master.local` | [node-1-dome-motion.yaml](../../node-1-dome-motion.yaml) |
| **Node 2** | `wee2d2-sound-hub.local` | [node-2-sound-hub.yaml](../../node-2-sound-hub.yaml) |
| **Node 3** | `wled-dome.local` | [node-3-segments.json](../../node-3-segments.json) |
...
Successful deployment is confirmed by the appearance of the "Diagnostic" cards in Home Assistant.
