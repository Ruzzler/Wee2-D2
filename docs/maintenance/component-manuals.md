# 📚 IMPERIAL DATABANK: MANUAL REGISTRY
> **TECHNICAL REPOSITORY**

Welcome to the centralized Technical Manuals Registry. This page contains all the deep-level datasheets and specifications for the hardware systems powering Wee2-D2.

---

## ⚡ Energy & Distribution
*   [MgcSTEM LVP-R1.5 (40A Cutoff)](../hardware/mgcstem-lvp-r15-manual.md) — [[Source PDF](../hardware/mgcstem-lvp-r15-manual.pdf)]
*   [CNBTR 6-Circuit Slip Ring](../hardware/cnbtr-slip-ring-manual.md)

## 🛞 Motive Drives
*   [Flipsky Mini FSESC 6.7 Pro](../hardware/flipsky-fsesc-67-pro-manual.md) — [[Source PDF](../hardware/flipsky-fsesc-67-pro-manual.pdf)]
*   [L-faster FLD-5 Hub Motor](../hardware/hub-motor-manual.md)
*   [goBILDA 1x15A PWM ESC](../hardware/gobilda-motor-manual.md)
*   [goBILDA 5203 Yellow Jacket Gearmotor](../hardware/gobilda-motor-manual.md)

## 📡 Signal & Audio
*   [HOTRC DS-600 Transmitter](../hardware/hotrc-ds600-manual.md) — [[Source PDF](../hardware/hotrc-ds600-manual.pdf)]
*   [HOTRC F-06A PWM Receiver](../hardware/hotrc-f06a-manual.md) — [[Source PDF](../hardware/hotrc-f06a-manual.pdf)]
*   [DFPlayer Mini (MP3-TF-16P)](../hardware/dfplayer-mini-manual.md)
*   [GrnWave Circular PSI Logic](../hardware/grnwave-psi-manual.md)

---

> [!TIP]
> Need to see exactly how these parts are orchestrated together? Head back to the [Interactive Electrical Schematic](../architecture/electrical-schematic.md) or look up required quantities in the [Bill of Materials](../bill-of-materials.md).
