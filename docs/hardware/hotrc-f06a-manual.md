...
> **TECHNICAL SPECIFICATIONS** | **6-CHANNEL RX** | **PWM SIGNAL OUTPUT**
...
1. Power on the F-06A receiver (supply 5V to any VCC/GND pin pair). The small LED on the receiver will flash rapidly, indicating it is in bind mode.
1. Turn **ON** the DS-600 transmitter.
1. The receiver's LED will stop flashing and turn solid red, indicating a successful bind.
...
1. Set the transmitter trigger and joystick to the completely neutral (center) positions.
1. Insert a standard bind plug (or bridge the signal to ground) on the **"B" (Bind)** port of the receiver for 3 seconds.
1. The receiver will save these neutral PWM values. If the signal drops, it will automatically output these saved values to the ESP32 and stop the droid.
